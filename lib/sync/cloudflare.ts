...
export async function fetchMeta(userId: string): Promise<Meta> {
  ...
  if (!r.ok) throw new Error('meta'); return r.json();
}

export async function initVault(userId: string, kdf: any, wrappedVaultKey: string): Promise<{ etag: string }> {
  const r = await fetch(`${WORKER_BASE}/v1/init`, { method:'POST', headers:{'content-type':'application/json'}, body: JSON.stringify({ userId, kdf, wrappedVaultKey })});
